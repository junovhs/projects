import { formatNumber, inchesToUnit, PRESETS, unitToInches } from "./config";
...
function setBleed(state: AppState, dom: DomRefs): void {
	...
	state.bleedIn = Math.max(0, unitToInches(value, state.units));
...
function setLive(state: AppState, dom: DomRefs): void {
	...
	state.liveIn = Math.max(0, unitToInches(value, state.units));
}

function setInset(state: AppState, dom: DomRefs): void {
	const value = parseFloat(dom.insetRange.value) || 0;
	state.insetIn = Math.max(0, unitToInches(value, state.units));
}
...
function updateDimInputs(state: AppState, dom: DomRefs): void {
...
function updateAllUnitInputs(state: AppState, dom: DomRefs): void {
	updateDimInputs(state, dom);

	dom.bleedRange.value = String(inchesToUnit(state.bleedIn, state.units));
	dom.bleedNum.value = formatNumber(
		inchesToUnit(state.bleedIn, state.units),
		3,
	);
	dom.liveRange.value = String(inchesToUnit(state.liveIn, state.units));
	dom.liveNum.value = formatNumber(inchesToUnit(state.liveIn, state.units), 3);
	dom.insetRange.value = String(inchesToUnit(state.insetIn, state.units));
...
function setupSizeControls(
	...
	dom.size.addEventListener("change", () => {
		state.documentSize = dom.size.value as PresetKey;
	...
	dom.units.addEventListener("change", () => {
		state.units = dom.units.value as UnitKey;
		updateAllUnitInputs(state, dom);
		redraw();
	});
...
function setupActions(state: AppState, dom: DomRefs, redraw: () => void): void {
	...
	dom.exLine.addEventListener("click", () => {
		const res = exportSvg("line", state);
		if (!res.ok) console.error(res.error);
	});

	dom.exFill.addEventListener("click", () => {
		const res = exportSvg("fills", state);
		if (!res.ok) console.error(res.error);
	});

	dom.btnDelCut.addEventListener("click", () => {
		if (state.selectedCutIndex !== null && state.selectedCutIndex >= 0) {
