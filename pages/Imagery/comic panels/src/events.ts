import { AppState, DomRefs } from "./types"; import { distancePointToSegment, clampPoint, pointInPolygon } from "./geometry"; import { previewDimsPx } from "./layout"; import { splitPanelVertical, splitPanelHorizontal, deletePanelById } from "./panels";
...
const dims = previewDimsPx(state);
const pos = clampPoint(
canvasPos(dom.canvas, e),
dims.cw,
dims.ch
);
...
if (state.dragMode && state.currentCut) {
  const idx = state.cuts.findIndex(
    (c) => c.id === state.currentCut?.id
  );
  if (idx < 0) {
    return;
  }
...
if (state.isDragging && state.currentCut) {
const dx =
state.currentCut.end.x - state.currentCut.start.x;
const dy =
state.currentCut.end.y - state.currentCut.start.y;

  if (Math.hypot(dx, dy) > 10) {
    state.currentCut.id =
      "c" + String(state.idSeq);
    state.idSeq += 1;
...
for (const panel of state.panels) {
  const inside = pointInPolygon(pos, panel.points);
  if (!inside) {
    continue;
  }
  ...
  if (!state.mergeSel.includes(panel.id)) {
    const maxPanels = 8;
    const full = state.mergeSel.length >= maxPanels;
    if (full) {
      state.mergeSel.shift();
    }
    state.mergeSel.push(panel.id);
  }
