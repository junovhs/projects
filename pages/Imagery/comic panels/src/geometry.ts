...
const a = poly[i];
const b = poly[j];
const intersectY = (a.y > pt.y) !== (b.y > pt.y);

```
if (intersectY) {
  const x =
    ((b.x - a.x) * (pt.y - a.y)) / (b.y - a.y + 1e-12) + a.x;
  if (pt.x < x) {
    inside = !inside;
  }
}
...
const high = clipHalfPlane(poly, start, nx, ny, +offset, false);
const low = clipHalfPlane(poly, start, nx, ny, -offset, true);
