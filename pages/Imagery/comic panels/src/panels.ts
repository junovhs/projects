import { polygonBounds, splitPolygonByStrip } from "./geometry";
import { computeBaseRects } from "./layout";
import { nextId } from "./state";
import type { AppState, BaseRects, Cut, Panel, Point } from "./types";
...
export function recomputePanels(state: AppState): void {
	...
	for (const cut of state.cuts) {
		...
		for (const panel of acc) {
			const appliesToPanel = cut.type === "global" || cut.panelId === panel.id;
...
function overlapLength(p1: Point, p2: Point, q1: Point, q2: Point): number {
	const v1x = p2.x - p1.x;
	const v1y = p2.y - p1.y;
	const v2x = q2.x - q1.x;
	const v2y = q2.y - q1.y;
...
function edgeShared(a: Panel, b: Panel): boolean {
	...
	for (const segA of ea) {
		for (const segB of eb) {
			const len = overlapLength(segA[0], segA[1], segB[0], segB[1]);
			if (len > threshold) {
				return true;
			}
		}
...
function connectedSubset(ids: string[], adj: Adjacency): string[] {
	if (!ids.length) {
		return [];
	}
...
function boundsOverlap(cut: Cut, panel: Panel, gutter: number): boolean {
	...
	const noOverlap = b.x1 > maxX || b.x2 < minX || b.y1 > maxY || b.y2 < minY;
...
function scopeGlobalCut(state: AppState, cut: Cut): void {
...
export function mergeSelectedPanels(state: AppState): void {
	...
	const selectedPanels = state.panels.filter((p) => selectedSet.has(p.id));
	...
	for (let i = 0; i < selectedPanels.length; i += 1) {
		for (let j = i + 1; j < selectedPanels.length; j += 1) {
			...
			for (const cut of cutsCopy) {
				const separates = cutSeparatesPanels(cut, a, b, state.gutterPx);
				if (!separates) {
					continue;
				}
...
export function splitPanelVertical(state: AppState, panelId: string): void {
...
export function splitPanelHorizontal(state: AppState, panelId: string): void {
	const panel = state.panels.find((p) => p.id === panelId);
	if (!panel) {
		return;
	}
...
export function deleteCutAtIndex(state: AppState, index: number): void {
	state.cuts = state.cuts.filter((_, i) => i !== index);
	state.selectedCutIndex = null;
	state.showDeleteButton = false;
}

export function deletePanelById(state: AppState, panelId: string): void {
